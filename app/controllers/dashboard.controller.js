const NotionService = require('../services/notion.server');
...
module.exports = class DashboardController {
  static async getItems(req, res) {
    if (startingTime < new Date().setHours(0, 0, 0, 0))
      dbs = NotionService.getItems();

    const minimal = (await dbs).results.reduce((result, page) => {
      if (page.properties.Name.title.plain_text !== 'Morning')
        result.push({
